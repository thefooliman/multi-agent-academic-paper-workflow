...
(humanizer-patterns-en.md Group 7), to argue rather than survey (Group 8), and
to hold the line that every claim earns its number or citation and no verb
...
2. Audit without editing: list each tell with its location and fix, and each
   ...
   pass, including the additive ones (Group 7 missing texture, Group 8 content
   stance), not just the removal patterns.
